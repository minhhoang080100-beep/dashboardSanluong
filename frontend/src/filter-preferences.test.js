...
test('filter preferences stay separate for each internal user', (t) => {
  ...
  const filters = { start_date: '2026-08-01', end_date: '2026-08-31', terminal: 'ben_thuy' };
  rememberFilters(first, filters);
  assert.deepEqual(restoreFilters(first), filters);
  assert.deepEqual(restoreFilters(second), { start_date: '2026-09-01', end_date: '2026-09-13', terminal: 'all' });
...
test('a reduced terminal scope discards a previously saved wider filter', (t) => {
  ...
  rememberFilters(user, { start_date: '2026-08-01', end_date: '2026-08-31', terminal: 'all' });
  assert.deepEqual(restoreFilters({ ...user, terminals: ['ben_thuy'] }), { start_date: '2026-09-01', end_date: '2026-09-13', terminal: 'ben_thuy' });
...
test('corrupt, impossible, future, overlong and unauthorized saved filters revert to current scoped defaults', (t) => {
  ...
  const defaults = { start_date: '2026-09-01', end_date: '2026-09-13', terminal: 'cua_lo' };
  const base = { start_date: '2026-08-01', end_date: '2026-08-31', terminal: 'cua_lo' };
...
test('unavailable preference storage does not prevent loading or applying reports', (t) => {
  ...
  assert.deepEqual(restoreFilters(user), { start_date: '2026-09-01', end_date: '2026-09-13', terminal: 'ben_thuy' });
});
...
test('today and yesterday presets follow Vietnam midnight across the year boundary', (t) => {
  ...
  assert.deepEqual(restoreFilters({ id: 1, terminals: ['cua_lo'] }), { start_date: '2027-01-01', end_date: '2027-01-01', terminal: 'cua_lo' });
