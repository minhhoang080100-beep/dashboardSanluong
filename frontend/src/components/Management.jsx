...
import { Check, ClipboardList, Download, Eye, EyeOff, KeyRound, Plus, RefreshCw, Trash2, Upload } from 'lucide-react';
...
function PlanImport({ apiBase, onImported, terminals, id, editorRef, open, onOpenChange }) {
  ...
  return <details id={id} ref={editorRef} className="management-editor plan-entry-editor" hidden={!open} open={open} onToggle={(event) => onOpenChange(event.currentTarget.open)}><summary onClick={(event) => { if (task.busy) event.preventDefault(); }}><span>Nhập kế hoạch từ Excel</span><small>Bấm để thu gọn</small></summary><form className="management-upload" onSubmit={previewFile}>
...
function PlansPanel({ user, filters, apiBase, onSelectPlan, preferredPeriodType }) {
  ...
  const importEditor = useRef(null);
  ...
  function changeCreateOpen(open) {
    setCreateOpen(open);
    if (!open && createOpen) createTrigger.current?.focus();
  }
  function changeImportOpen(open) {
    setImportOpen(open);
    if (!open && importOpen) importTrigger.current?.focus();
  }
  function openImport() {
    setImportOpen(true);
    requestAnimationFrame(() => { importEditor.current?.scrollIntoView({ block: 'nearest' }); importEditor.current?.querySelector('input[type="file"]')?.focus(); });
  }
  ...
  return <div className="management-stack">
    <section className="management-card plans-section"><div className="management-heading plan-list-heading"><div className="plan-list-title"><h3 ref={listHeading} tabIndex={-1}>Danh sách kế hoạch</h3><p className="plan-list-subtitle">Cảng Nghệ Tĩnh · Các kỳ và phiên bản</p></div><div className="management-actions plan-header-actions">{canManage(user) && <><button ref={createTrigger} className="button primary" type="button" aria-expanded={createOpen} aria-controls={`${panelId}-create`} disabled={task.busy || !terminals.length} onClick={() => openCreate()}><Plus size={15} aria-hidden="true" />Tạo kế hoạch</button><button ref={importTrigger} className="button" type="button" aria-expanded={importOpen} aria-controls={`${panelId}-import`} onClick={openImport}><Upload size={15} aria-hidden="true" />Nhập Excel</button></>}<button className="button" type="button" onClick={refresh} disabled={result.resource.loading}><RefreshCw size={15} aria-hidden="true" />Tải lại</button></div></div>
      ...
      </form></details><PlanImport id={`${panelId}-import`} editorRef={importEditor} open={importOpen} onOpenChange={changeImportOpen} apiBase={apiBase} terminals={terminals} onImported={refresh} /></>}
      ...
      {result.resource.data && (result.items.length ? <><div className="table-scroll plans-table-scroll" tabIndex={0} role="region" aria-label="Bảng danh sách kế hoạch"><table className="plans-table" role="table">
        ...
        <thead role="rowgroup"><tr role="row">{[['Kỳ / xí nghiệp', 'plan-period-cell'], ['Chỉ tiêu'], ['Kế hoạch', 'plan-amount-cell'], ['Phiên bản', 'plan-version-cell'], ['Trạng thái'], ['Văn bản', 'plan-reference-cell'], ['Thao tác', 'plan-actions-cell']].map(([label, className]) => <th key={label} className={className} scope="col" role="columnheader">{label}</th>)}</tr></thead>
        <tbody role="rowgroup">{result.items.map((plan) => <tr role="row" key={plan.id} id={`saved-plan-${plan.id}`} tabIndex={-1} className={[lastCreated?.id === plan.id && 'plan-saved-row', plan.is_deleted && 'plan-deleted-row'].filter(Boolean).join(' ') || undefined}>
          ...
          <td role="cell" className="plan-version-cell"><span className="plan-cell-label" aria-hidden="true">Phiên bản</span>{plan.version}</td>
...
function ReconciliationPanel({ user, filters, report, apiBase, onReportChange }) {
  ...
  return <div className="management-stack"><section className="management-card"><div className="management-heading"><h3>Đối soát dữ liệu nguồn</h3>{onReportChange && <button className="button" type="button" onClick={onReportChange}><RefreshCw size={15} />Tải báo cáo mới</button>}</div>
    {!reportId ? <p className="management-empty">Tải báo cáo sản lượng để xem các dòng cần đối soát.</p> : <>
      ...
      {displayedResource.data && (rows.length ? <><div className="table-scroll" tabIndex={0} role="region" aria-label="Bảng tác nghiệp cần đối soát"><table className="management-operations"><thead><tr><th>Mã / ngày</th><th>Xí nghiệp / tàu</th><th>Hàng hóa</th><th>Số lượng nguồn</th><th>Trọng lượng nguồn</th><th>Tấn</th><th>Đối soát</th></tr></thead><tbody>{rows.map((row) => {
