...
import { berthAssignmentLabel, initialBerthLabel, productionScopeLabel } from '../production-scope';
import './Voyages.css';
...
function VoyageDialog({ selected, filters, apiBase, onClose, trigger, reportId, onReportReload }) {
  ...
  const requestKey = `${reportId || ''}/${filters.production_scope}/${selected.terminal_id}/${selected.voyage_id}/${filters.start_date}/${filters.end_date}/${operationFilter}/${page}`;
  ...
  async function exportVoyage() {
    setExportError('');
    const outcome = await download.run(`/reports/${encodeURIComponent(reportId)}/export.xlsx?${new URLSearchParams({ terminal: selected.terminal_id, voyage_id: selected.voyage_id, production_scope: filters.production_scope, operation_filter: operationFilter })}`, `chuyen-tau-${filters.production_scope}-${selected.terminal_id}-${selected.voyage_id}.xlsx`, { baseUrl: apiBase });
    if (outcome.error) {
  ...
  return <dialog ref={dialog} className="voyage-dialog" aria-label="Chi tiết chuyến tàu" onCancel={(event) => { event.preventDefault(); onClose(); }}>
    ...
    <div className="voyage-dialog-body">
        <p className="voyage-detail-period">{productionScopeLabel(filters.production_scope)} · Kỳ tác nghiệp: {formatDate(filters.start_date)} – {formatDate(filters.end_date)} · ID chuyến dùng khi nhập kế hoạch: {selected.voyage_id}</p>
      {reportId && <div className="voyage-detail-actions"><button type="button" className="button" disabled={!data || download.busy} aria-busy={download.busy} onClick={exportVoyage}><Download size={16} aria-hidden="true" />{download.busy ? 'Đang xuất Excel…' : 'Xuất Excel chuyến tàu'}</button></div>}
      ...
      {data && <>
        <dl className="voyage-metadata"><div><dt>Tàu</dt><dd>{data.header.vessel_name || 'Chưa có tên tàu'}</dd></div><div><dt>Mã chuyến</dt><dd>{data.header.voyage_code || data.header.voyage_id}</dd></div><div><dt>Ngày làm hàng trong kỳ</dt><dd>{dateRange(data.header.first_operation_date, data.header.last_operation_date)}</dd></div><div><dt>Đến cảng thực tế</dt><dd>{timestamp(data.header.arrival_at)}</dd></div><div><dt>Rời cảng thực tế</dt><dd>{timestamp(data.header.departure_at)}</dd></div><div><dt>Xí nghiệp</dt><dd>{data.header.terminal_name}</dd></div><div><dt>Cầu ban đầu</dt><dd>{initialBerthLabel(data.header)}<small>{berthAssignmentLabel(data.header.berth_assignment_status)}</small></dd></div><div><dt>Thời điểm cầu ban đầu</dt><dd>{timestamp(data.header.initial_berth_at)}</dd></div><div><dt>Phạm vi toàn chuyến</dt><dd>{productionScopeLabel(data.header.production_scope)}</dd></div></dl>
        <div className="voyage-summary"><div><span>Sản lượng thông qua</span><strong>{formatNumber(data.summary.tonnage)} <small>tấn</small></strong></div><div><span>Container</span><strong>{formatNumber(data.summary.teu)} <small>TEU</small></strong></div><div><span>Dòng có phát sinh</span><strong>{formatNumber(data.operations.counts.with_values, 0)}</strong><small className="voyage-source-count">{formatNumber(data.operations.total_all, 0)} dòng nguồn</small></div></div>
        ...
        {reportId && <VoyageProgress selected={selected} productionScope={filters.production_scope} apiBase={apiBase} />}
        ...
        <section className="voyage-operations" aria-labelledby="voyage-operations-title">
          ...
          {data.operations.rows.length > 0 ? <>
            <div className="table-scroll operations-scroll" tabIndex={0} role="region" aria-label="Tác nghiệp của chuyến tàu"><table className="operations-table" role="table"><caption className="sr-only">Danh sách dòng tác nghiệp của chuyến tàu theo bộ lọc đang chọn</caption><thead role="rowgroup"><tr role="row"><th scope="col" role="columnheader">Mã tác nghiệp / ngày</th><th scope="col" role="columnheader">Hàng hóa / phương án</th><th scope="col" role="columnheader">Hướng hàng</th><th scope="col" role="columnheader">Số lượng nguồn</th><th scope="col" role="columnheader">Trọng lượng nguồn</th><th scope="col" role="columnheader">Tấn</th><th scope="col" role="columnheader">TEU</th></tr></thead><tbody role="rowgroup">{data.operations.rows.map((row) => <tr role="row" key={row.id}><th scope="row" role="rowheader">{row.operation_code || row.id}<small>{formatDate(row.operation_date)}</small>{row.shift_code && <small>Ca {row.shift_code}</small>}</th><td role="cell" data-label="Hàng hóa / phương án" className="operation-context">{row.cargo_name || '—'}<small>{row.job_method || '—'}</small></td><td role="cell" data-label="Hướng hàng" className="operation-context">{row.direction || '—'}</td><td role="cell" data-label="Số lượng nguồn">{formatNumber(row.quantity)}<small>{row.quantity_unit_name || row.quantity_unit || '—'}</small></td><td role="cell" data-label="Trọng lượng nguồn">{formatNumber(row.weight)}<small>{row.weight_unit_name || row.weight_unit || '—'}</small></td><td role="cell" data-label="Tấn">{formatNumber(row.tonnage)}</td><td role="cell" data-label="TEU">{formatNumber(row.teu)}</td></tr>)}</tbody></table></div>
...
export default function Voyages({ rows, count, filters, apiBase, onRetry, reportId }) {
  ...
  return <section className="panel voyages-panel" id="voyages" aria-labelledby="voyages-title">
    ...
    {results.items.length > 0 ? <>
      <div className="table-scroll voyage-table-scroll"><table className="voyage-table"><caption className="sr-only">Các chuyến tàu có tác nghiệp trong kỳ, xí nghiệp và phạm vi sản lượng đang chọn</caption><thead><tr><th scope="col">Tàu / chuyến</th><th scope="col">Xí nghiệp</th><th scope="col">Cầu ban đầu</th><th scope="col">Ngày làm hàng</th><th scope="col">Tấn</th><th scope="col">TEU</th><th scope="col"><span className="sr-only">Chi tiết</span></th></tr></thead><tbody>{results.items.map((row) => <tr key={`${row.terminal_id}/${row.voyage_id}`}><th scope="row" className="voyage-name"><strong>{row.vessel_name || 'Chưa có tên tàu'}</strong><small>{row.voyage_code || row.voyage_id}</small></th><td data-label="Xí nghiệp">{row.terminal_name}</td><td data-label="Cầu ban đầu" className="voyage-berth">{initialBerthLabel(row)}{row.berth_assignment_status !== 'assigned' && <small>{berthAssignmentLabel(row.berth_assignment_status)}</small>}</td><td data-label="Ngày làm hàng">{dateRange(row.first_operation_date, row.last_operation_date)}</td><td data-label="Tấn">{formatNumber(row.tonnage)}</td><td data-label="TEU">{formatNumber(row.teu)}</td><td className="voyage-open-cell"><button className="button voyage-open" type="button" aria-label={`Xem chi tiết ${row.vessel_name || 'Chưa có tên tàu'} · ${row.voyage_code || row.voyage_id} · ${row.terminal_name}`} onClick={(event) => { trigger.current = event.currentTarget; setSelected(row); }}>Xem chi tiết<ArrowRight size={14} aria-hidden="true" /></button></td></tr>)}</tbody></table></div>
