...
def report_fixture(filters):
    ...
    # One grouped fact represents 26 source rows for the pagination scenario.
    for voyage in data["voyages"]:
        voyage["record_count"] = 26
    data["overview"]["record_count"] = 26 * len(data["voyages"])
    return data


def detail_fixture(filters, terminal, page, page_size):
    ...
    coverage["tonnage"].update(eligible_rows=26, known_rows=26)
    return {
        "header": header,
        ...
        "operations": {"page": page, "page_size": page_size, "total": 26,
                       "total_pages": (26 + page_size - 1) // page_size,
                       "rows": rows[(page - 1) * page_size:page * page_size]},
        "meta": {"filters": {**filters, "terminal": terminal, "voyage_id": "101", "timezone": "Asia/Ho_Chi_Minh"},
...
def main():
    ...
    def respond_detail(route):
        ...
        request = {**filters, "terminal": terminal, "voyage_id": voyage_id, "page": page}
        ...
        if state["detail"] == "error":
            ...
            held.append((route, filters, terminal, page, page_size))
        else:
            route.fulfill(json=detail_fixture(filters, terminal, page, page_size))
    ...
    with sync_playwright() as p:
        ...
        state["detail"] = "data"
        for route, filters, terminal, number, size in held:
            try:
                route.fulfill(json=detail_fixture(filters, terminal, number, size))
            except Error:
                pass
        expect(dialog).not_to_be_visible()
        expect(page.locator(".voyages-panel")).not_to_contain_text("CUA LO TEST")
        checks.append("scope race: closed detail cannot reopen after the global filter changes")
